...
reset() { curl -fsS -X PATCH "$API/proxies/mqtt" -d '{"enabled":true}' >/dev/null  # undo an interrupted outage
...
case "${1:-}" in
  ...
  outage)  curl -fsS -X PATCH "$API/proxies/mqtt" -d '{"enabled":false}' >/dev/null; sleep "${2:-60}"
           curl -fsS -X PATCH "$API/proxies/mqtt" -d '{"enabled":true}'  >/dev/null ;;
  reset)   reset ;;
