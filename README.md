...
> **Verified so far:** CI runs the harness tests, checks fixtures regenerate byte-identical, and builds and import-checks the `sim` and `ingest` images. `make up` has been booted end to end: events reach Postgres and every Grafana panel fills. `make drill` has been run on a fresh lab: through a 64 kbps cap, 400 ms latency each way and a 2 minute outage, all 1388 events of the 5 scenes that finished arrived exactly once (naive-event delivery lag: median 0.59 s under latency, up to 122.54 s after the outage). `make broker-restart` has been run on a fresh lab: with ingest away for 60 s across a broker restart, all 846 events of the 3 finished scenes arrived exactly once. `make up-video` has been booted on a short handheld stock clip: `edge-01` events reach Postgres and Grafana, but zone accuracy is not measured because the camera moves. `make visuals` and `make trackers` are not run in CI.
